...
### Cotizaciones — cliente y agencia, eliminar, categorías por nombre (sep-2026)
- **Encabezado**: `cliente_id` / `cliente_nombre_libre` = CLIENTE FINAL (la marca); `agencia_id` / `agencia_nombre_libre` (`sql/cotizaciones_agencia.sql`) = intermediaria opcional. `cliente_final` es HEREDADO (modelo viejo: agencia en cliente_*, marca en cliente_final). Una sola regla de lectura, `encabezadoCotizacion()` / `etiquetaEncabezado()` en `lib/cotizaciones-encabezado.ts`, cubre ambos modelos: la usan el PDF, la vista del cliente, el listado, `clienteCot()` y el agente. Con dos FKs a `clientes`, todo select sobre `cotizaciones` debe desambiguar: `cliente:clientes!cliente_id(...)`, `agencia:clientes!agencia_id(...)`.
- **Eliminar cotización**: borrado real y reversible (`lib/cotizaciones-eliminar.ts`: bloqueos por rendiciones/gastos rendidos/rodajes, captura del árbol, borrado; el grupo numerado cae si era la última versión; el número no se reutiliza). UI: botón "eliminar" en el editor → historial en ruta `/cotizaciones` (Ctrl+Z desde el listado). Agente: `hilvan_cotizacion_eliminar` → `hilvan_deshacer` reinserta el árbol.
- **Categorías por nombre**: `lib/agent-categorias.ts` (`resolverCategoria`, `obtenerOCrearCategoria`): uuid o nombre sin mayúsculas/tildes dentro de una cotización; ambiguo → error con ids. La usan `cotizacion-categoria`, `cotizacion-precio-categoria` y `cotizacion-editar-item` (mover con `departamento`/`subgrupo`, `subgrupo: null` = directo). `hilvan_cotizacion_detalle` trae ids en cada nodo, `vacio`, textos (`incluir_textos`) y encabezado.
- **Regla de boleta**: `con_boleta=false` ⇒ `tasa_boleta=0` (UI, `validarItem`, `cotizacion-editar-item`); limpieza de datos en `sql/cotizaciones_agencia.sql`.
- **Equipos por MCP** (solo lectura): `hilvan_equipos_listar` (`/api/agent/equipos`) y `hilvan_bundles_listar` (`/api/agent/bundles`: los kits = equipos de categoría KIT + composición de `lib/rental-kits.ts`). La tabla `bundles`/`bundle_items` es otra cosa (2 maletas sin precio) y no es lo que muestra el rental.
