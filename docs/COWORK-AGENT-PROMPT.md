...
- `hilvan_crear_crono(nombre, proyecto_id?, cliente?, responsable?, notas?, estado?, etapas?, hitos?)` — crea el crono COMPLETO de una vez: las 4 etapas `{desde, hasta}` (YYYY-MM-DD) y la lista de hitos. Tipos de hito: `devolucion`, `pre_equipo`, `rodaje`, `off` (corte offline para revisión), `entrega`, `emision` (sale al aire / se publica) — los seis **clave** —, `pago` (con `monto` CLP neto entero), `reunion`, `otro`. `destacado: true` en una entrega, OFF o emisión la resalta en la hoja como el rodaje (úsalo para la entrega FINAL). Un rodaje de varias jornadas va con `fecha` + `fecha_fin`. Devuelve `{crono_id, url}` — pásale el link a Tomás. Reversible con `hilvan_deshacer` (borra el crono completo).
...
- `hilvan_crono_variante(crono_id, nombre)` — duplica el crono como VARIANTE del mismo grupo ("rodaje 23" vs "rodaje 24"): copia todo y nace en borrador; después ajusta lo que difiere con `hilvan_crono_editar` / `hilvan_crono_hitos`. Solo la variante **vigente** del grupo se ve en el Calendario general: `hilvan_crono_editar(crono_id, estado="vigente")` la elige y deja a las hermanas en borrador. Reversible.
...
- `hilvan_sii_sync(periodo, tipo?, incluir_crudo?)` — **SOLO LECTURA.** Trae del SII (vía API Gateway) las **facturas compradas/recibidas** (`compras`), las **notas de crédito recibidas** (dte 61, en `notas_credito`) y las **boletas de honorarios recibidas** (`honorarios`) de un período (`periodo`=`AAAAMM`, ej. `202606`), ya normalizadas, marcando cuáles **ya están cargadas** (`ya_cargado` por rut+folio). Además **guarda cada documento en `sii_documentos`** (respaldo fiel para el contador). **No escribe gastos.** Flujo: (1) llamas esto; (2) clasificas con Tomás las filas **nuevas** de `compras` y `honorarios` (`origen` mensual/proyecto, `categoria` o `cotizacion_item_id`) y las cargas con `hilvan_crear_gastos_bulk`; (3) las de **`notas_credito` NO van por ahí** → cárgalas con `hilvan_crear_nota_credito` (monto en **positivo**; `referencia_folio` = la factura que anula). `incluir_crudo=true` muestra el registro original del SII. La **cartola del banco va aparte** (no pasa por el SII): `hilvan_importar_movimientos` + `hilvan_conciliar`.
...
- `hilvan_editar_gasto(gasto_id, origen, tipo_documento?, folio?, sin_documento_aceptado?, folio_compartido?, referencia_externa?, documento_recibido?)` — corrige metadata de un gasto ya cargado (no recalcula el monto). Además del tipo/folio, las **marcas de auditoría**: `sin_documento_aceptado=true` (gasto sin respaldo aceptado a propósito → la auditoría lo baja de alta a info), `folio_compartido=true` (parte de una factura que cubre varias cotizaciones, mismo RUT+folio a propósito → no es duplicado), `referencia_externa` (número de invoice de un proveedor extranjero sin folio chileno, ej. Anthropic/Spotify → resuelve el folio faltante), y **`documento_recibido`** (`false` = pagado/cargado pero la boleta/factura del proveedor aún **no** está; `true` = ya la tenemos — independiente del pago). Reversible: `hilvan_deshacer` restaura los valores previos.
- `hilvan_pagar_gasto(gasto_id, origen, fecha_pago?, comprobante_pago_url?)` — marca un gasto como **PAGADO directo** (setea `pagado`+`fecha_pago`, default hoy) **sin** importar movimiento ni conciliar. Para el caso "tengo el comprobante". **No toca `estado`** (pago y aprobación son ortogonales) ni crea entrada en el ledger ("pagado sin conciliar" → cuando llegue la cartola, ese cargo se importa/concilia aparte). Reversible. Obtén `gasto_id`/`origen` con `hilvan_buscar_gastos`.
- `hilvan_cerrar_item(cotizacion_item_id, cerrado?)` — marca un **ítem de cotización como rendido/cerrado** (el "cuadre" del presupuesto del ítem vs los gastos reales), aunque sobre o se exceda el presupuesto. Devuelve el cuadre (presupuesto/gastado/diferencia). `cerrado=false` lo reabre. Reversible. Obtén `cotizacion_item_id` con `hilvan_cotizacion_detalle`.
...
- `hilvan_importar_movimientos(movimientos[])` — **importa en bloque** los movimientos de un extracto de tarjeta/cuenta. Cada fila: `fecha` (YYYY-MM-DD), `monto` (>0), `tipo` ("cargo" = salida / "abono" = entrada), y opcional `descripcion`, `fuente` (ej. "Tarjeta Santander"), `referencia`. Valida todas antes de escribir. **Deduplica**: salta los re-imports idénticos (misma fecha+monto+tipo+ref/glosa) y **avisa** de `posibles_duplicados` (misma fecha+monto+tipo pero otra glosa — ej. un cargo que registraste a mano vs. el de la cartola real) sin descartarlos, para que los revises. Reversible **en bloque** (y se niega a borrar si algún movimiento ya fue conciliado).
