...
- `hilvan_sii_sync(periodo, tipo?, incluir_crudo?)` — **SOLO LECTURA.** Trae del SII (vía API Gateway) las **facturas compradas/recibidas** (RCV compras) y las **boletas de honorarios recibidas** de un período (`periodo`=`AAAAMM`, ej. `202606`), ya normalizadas al shape de gasto, y marca cuáles **ya están cargadas** en Hilván (`ya_cargado` por rut+folio). **No escribe nada.** Flujo: (1) llamas esto, (2) clasificas con Tomás cada fila **nueva** (`origen` mensual/proyecto, `categoria` o `cotizacion_item_id`), (3) cargas con `hilvan_crear_gastos_bulk`. Si los montos se ven raros, repite con `incluir_crudo=true` para ver el registro original del SII. La **cartola del banco va aparte** (no pasa por el SII): se importa con `hilvan_importar_movimientos` y se concilia con `hilvan_conciliar`.
