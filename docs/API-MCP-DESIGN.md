...
| GET | `/api/agent/estado-financiero?periodo=YYYY-MM` | `{periodo, ingresos:{facturado_periodo, cobrado_periodo, por_cobrar:{total, items:[{numero, cliente, monto, dias_aging}]}}, egresos:{total, por_origen:{proyecto, mensual}, por_categoria:{<cat>:monto}, pagado, adeudado}, creditos:{cuotas_periodo:[{credito, acreedor, monto, fecha_vencimiento, pagada}], total_cuotas_periodo, pagadas, pendientes}, flujo_varios:{entradas, salidas}, resumen:{resultado_devengado, caja_aprox}}` — resumen financiero del mes para narrar "cómo vamos". `periodo` default = mes actual. Reutiliza `calcularTotalCot` para los totales de cotización; egresos netos de NC (montos negativos restan); cuadre de gastos por `fecha_documento` con fallback a `created_at`. NO incluye PPM/Previred/IUSC/nómina (fuera de v1). |
...
| GET | `/api/agent/flujo-caja?periodo=&tipo=` | `[{id, descripcion, monto, fecha, tipo:"entrada"\|"salida", created_at, created_by}]` — filas de `flujo_caja_manual` (ingresos/gastos varios), incluidas las creadas al conciliar movimientos sin match (máx 200, desc por fecha). `periodo`=YYYY-MM (filtra por mes de `fecha`); `tipo`=`entrada`/`salida`. |
