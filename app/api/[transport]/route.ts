  ...
  (server) => {
    ...
    server.registerTool(
      'hilvan_proximos_seguimientos',
      {
        ...
        description: 'LA AGENDA DE CONTACTO: a quién le toca hoy y a quién dentro de `dias` (default 7). Corre sobre el mismo motor de cadencia que el digest matinal y la pantalla del CRM, así que sus números coinciden — úsala para planificar el día en vez de hilvan_digest_matinal {dry:true}. Ordenada por prioridad: primero quien respondió, después el más atrasado. Devuelve aparte `por_verificar`: los que están fuera de la agenda porque su ficha no es de fiar — resolverlos es trabajo, no se esconden. Y devuelve `cotejo`: cuándo se cotejaron los correos por última vez. SI `cotejo.avisar` ES TRUE, ESTA AGENDA NO ES CONFIABLE — está calculada sobre respuestas que no se registraron, así que puede haber gente esperando contestación marcada como si nunca hubiera hablado. Cotejá antes de escribir borradores; si no puedes cotejar, dilo en el reporte en vez de redactar a ciegas. Devuelve estado, ultimo_toque, dias_atraso y sin_respuesta. Excluye confirmado/descartado/nurture/en_frio y a los agotados (16 toques sin respuesta).',
    ...
    server.registerTool(
      'hilvan_misiones_listar',
      {
        ...
        description: 'Las misiones que Tomás YA eligió y están cargadas en Hilván, con si la persona las declaró cumplidas. Sin parámetros trae la semana en curso; desde=YYYY-MM-DD trae desde esa fecha. LLÁMALA ANTES de proponer: sin esto vuelves a proponer espacios que ya están tomados. `cumplida` te sirve para CALIBRAR lo que propones —si un tipo de misión no se cumple nunca, el problema es la misión— pero NO para llevar la cuenta de quién cumplió: eso sería un ranking y no existe tablero de cumplimiento.',
