...
import { calcularRetencion, type Rendicion, type RendicionGasto } from '@/types'
...
export default async function AdminRendicionesPage() {
  ...
  const todosGastos = (rendiciones as Rendicion[]).flatMap((r: Rendicion) => r.gastos || [])

  const gastosPorRevisar = todosGastos.filter((g: RendicionGasto) => g.estado === 'enviada' && g.origen === 'externo')
  const gastosPorPagar = todosGastos.filter((g: RendicionGasto) =>
    (g.origen === 'interno' && g.estado === 'enviada') ||
    (g.origen === 'externo' && g.estado === 'aprobada')
  )
  const totalPorPagar = gastosPorPagar.reduce((s: number, g: RendicionGasto) => {
    ...
    <div className="p-6 lg:p-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
        {[
          ...
          { label: 'Sin documento', value: todosGastos.filter((g: RendicionGasto) => g.tipo_documento === 'sin_documento').length, color: 'text-red-400' },
        ].map(stat => (
